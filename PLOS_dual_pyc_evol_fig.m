%% Dual Pycnocline Data Set Results
clear all, close all;
...
tic
[error_map]=eof_error_map(reshape(myrhos,3001*128,100),num_modes);
toc
...
%% Data set with error map  
    ...
    set(gcf,'DefaultLineLineWidth',4,'DefaultTextFontSize',def_font_size,...
...
for ii = 1:4
    ...
    text(40, 110, strcat('$t = \, $', int2str(times(ii))), 'Color','white', 'Interpreter','latex') 
    ...
    colormap(cmocean('balance')) %Thyng et al. 2016 perceptually uniform colormap.
            ...
            if make_figs
                figname = strcat('dual_pyc_evol_fig_results');
